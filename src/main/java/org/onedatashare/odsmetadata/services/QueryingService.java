...
public class QueryingService {
     ...
     * @return
     */
    public List<Long> queryUserJobIds(@NotNull String userId) {
        return this.jdbcTemplate.queryForList(QUERY_GETUSERJOBIDS, Long.class, userId);
    }
    ...
    public List<JobStatistic> queryGetUserJobsByDate(@NotNull String userId, Date date) {
        ...
        list = this.jdbcTemplate.query(QUERY_GETUSERJOBSBYDATE, (rs, rowNum) ->
                new JobStatistic(rs.getLong(JOB_EXECUTION_ID),
                        rs.getTimestamp(START_TIME), rs.getTimestamp(END_TIME),
                        Status.valueOf(rs.getString(STATUS).toLowerCase()), rs.getTimestamp(LAST_UPDATED)
    ...
    public List<Long> queryGetUserJobsByDateRange(@NotNull String userId, Date to, Date from) {
        return this.jdbcTemplate.queryForList(QUERY_GETUSERJOBSBYDATERANGE, Long.class, userId, from, to);
    }
    ...
    public List<List<String>> mapStringVal(List<JobStatistic> jobStatisticList) {
        Map<Long, List<String>> allStringVal = jobStatisticList
                .stream()
                .collect(Collectors.groupingBy(JobStatistic::getJobId, Collectors.mapping(JobStatistic::getStrVal, toList())));
        ...
        return allStringVal.entrySet().stream().map(Map.Entry::getValue).collect(Collectors.toList());
    }
    ...
    public Set<JobStatisticDto> getJobStatisticDtos(List<JobStatistic> anyJobStat) {
        ...
        for (int i = 0; i < anyJobStat.size(); i++) {
            long currJobId = anyJobStat.get(i).getJobId();
            Set<String> fileNamesByJobId = anyJobStat.stream().
                    filter(job -> job.getJobId() == currJobId).map(JobStatistic::getFileName).collect(Collectors.toSet());
            Set<Integer> readCountByJobId = anyJobStat.stream().
                    filter(job -> job.getJobId() == currJobId).map(JobStatistic::getReadCount).collect(Collectors.toSet());
            Set<Integer> writeCountByJobId = anyJobStat.stream().
                    filter(job -> job.getJobId() == currJobId).map(JobStatistic::getWriteCount).collect(Collectors.toSet());

            JobStatisticDto jobStatisticDto = new JobStatisticDto();
            jobStatisticDto.setJobId(anyJobStat.get(i).getJobId());
            jobStatisticDto.setStartTime(anyJobStat.get(i).getStartTime());
            jobStatisticDto.setEndTime(anyJobStat.get(i).getEndTime());
            jobStatisticDto.setStatus(anyJobStat.get(i).getStatus());
            jobStatisticDto.setLastUpdated(anyJobStat.get(i).getLastUpdated());
            jobStatisticDto.setReadCount(readCountByJobId);
            jobStatisticDto.setWriteCount(writeCountByJobId);
            jobStatisticDto.setFileName(String.join(", ", fileNamesByJobId));
            jobStatisticDto.setStrVal(jobParamDetails);
            jobStatisticDto.setJobMeasurements(new ArrayList<>());
